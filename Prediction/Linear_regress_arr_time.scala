...
object Linear_regress_arr_time extends App{
...
val data_1415 = prepFlightDelays("airline/Data1415.csv").map(rec => rec.feature_details._2)
val data_2016 = prepFlightDelays("airline/Data16.csv").map(rec => rec.feature_details._2)
...
//val save = valuesAndPreds.map{ case(v, p) =>var line = p.toDouble + "," + v.toDouble
  //line
